...
\end{Proof}
\begin{theorem}
	 [критерий асимптотической устойчивости Гурвица]
	 Уравнение $(2.7.1)$ асимптотически устойчиво $\Longleftrightarrow$ положительны все угловые миноры матрицы
	 \begin{equation*}
	 	\begin{pmatrix}
	 		a_{n-1} & a_{n-3} & a_{n-5} & \ldots & \ldots \\
	 		1 & a_{n-2} & a_{n-4} & \ldots & \ldots \\
	 		0 & a_{n-1} & a_{n-3} & \ldots & \ldots \\
	 		\vdots & \vdots & \vdots & \ddots & \vdots \\
	 		\ldots & \ldots & \ldots & \ldots & a_0
	 	\end{pmatrix}
	 \end{equation*}
\end{theorem}
...
$$F_i(t,x_1(t), Dx_1, \ldots, D^{m_1}x_1, x_2(t), Dx_2,\ldots,D^{m_2}x_2,\ldots,x_k(t),Dx_k,\ldots,D^{m_k}x_k) = 0,$$ $$i = 1,\ldots,s,\quad t\in \I\subseteq \Rm,$$ где $F_i$ --- некоторая функция от своих $(m_1+\ldots+m_k + k + 1)$ переменных.}\\\\
...
\end{Proof}
\begin{cor}
	Общее решение уравнения $(3.4.1)$ имеет вид $$X_\text{оо}(t) = e^{A(t-t_0)}\cdot C,$$ где  $C$ -- это столбец произвольных постоянных вида $C = \begin{pmatrix}
		C_1\\\vdots\\C_n
	\end{pmatrix}.$
\end{cor}
\section*{Вычисление матричной экспоненты.}
Рассмотрим матрицу $A \in \mathbb R_{n,n}$.
\begin{theorem}
	Если $A$ -- это диагональная матрица вида $A = \operatorname{diag}\{\alpha_1,\ldots, \alpha_n\}$, то $$e^{At} = \operatorname{diag}\{e^{\alpha_1 t},\ldots, e^{\alpha_nt}\}.$$
\end{theorem}
\begin{Proof}
	...
	\begin{multline*}
		e^{At} = \begin{pmatrix}
			...
			0 & 1
		\end{pmatrix}+
		\dfrac{t}{1!} \cdot \begin{pmatrix}
			\alpha_1 & 0 \\
			0 & \alpha_2
		\end{pmatrix} +
		\dfrac{t^2}{2!}\cdot \begin{pmatrix}
			\alpha_1^2 & 0 \\
			0 & \alpha_2^2
		\end{pmatrix} + \ldots =\\= \begin{pmatrix}
		1 + \dfrac{1}{1!}\alpha_1 t + \dfrac{1}{2!} \alpha_1^2 t^2 + \ldots & 0 \\[2ex]
		0 & 1 + \dfrac{1}{1!}\alpha_2 t + \dfrac{1}{2!} \alpha_2^2 t^2 + \ldots
		\end{pmatrix} = \begin{pmatrix}
		e^{\alpha_1 t} & 0\\
		0 & e^{\alpha_2 t}
		\end{pmatrix}.
...
\end{Proof}
\begin{theorem}
	Если $A$ -- это блочно-диагональная матрица вида $A = \operatorname{diag}[A_1,\ldots, A_n]$, то $$e^{At} = \operatorname{diag}[e^{A_1t},\ldots, e^{A_nt}].$$
\end{theorem}
\begin{Proof}
	Аналогично предыдущей теореме.
\end{Proof}
\begin{theorem}
	Если $A$ является жордановой клеткой порядка $k$, соответствующей собственному значению $\alpha$, то есть $$A = J_k(\alpha) = \begin{pmatrix}
		\alpha & 1 & \dots & 0\\
...
\end{equation*}
\begin{Proof}
	Обозначим $$B = A - \alpha E = \begin{pmatrix}
		0 & 1 & \dots & 0\\
		\vdots & \vdots & \ddots & \vdots\\
		0 & 0 & \dots & 1\\
		0 & 0 & \dots & 0
	\end{pmatrix}.$$
	Так как матрица $E$ перестановочна с любой матрицей, то $\alpha E$ также перестановочна с любой матрицей. Следовательно, 
	\begin{equation*}
		e^{At} = e^{(A - \alpha E)t - \alpha E t} = e^{Bt + \alpha E t} = e^{Bt}\cdot e^{\alpha E t}.
	\end{equation*}
	При этом
	\begin{equation*}
		\alpha E t = \operatorname{diag}\{\alpha t,\ldots, \alpha t\},
	\end{equation*}
	а тогда
	\begin{equation*}
		e^{\alpha E t} = \operatorname{diag}\{e^{\alpha t},\ldots, e^{\alpha t}\} = e ^{\alpha t}\cdot E.
	\end{equation*}
	В свою очередь, 
	\begin{equation*}
		\begin{gathered}
			...
			0 & 0 & 0 & \dots & 0\\
			0 & 0 & 0 & \dots & 1
		\end{pmatrix} + \dfrac{t}{1!} \cdot \begin{pmatrix}
		0 & 1 & 0 & \dots & 0\\
		\vdots & \vdots & \ddots & \vdots\\
		0 & 0 & 0 & \dots & 0\\
		0 & 0 & 0 & \dots & 1\\
		...
		\end{pmatrix} +\ldots =\\
		=\begin{pmatrix}
			1 & \dfrac{t}{1!} & \dfrac{t^2}{2!} & \dots & \dfrac{t^{k-1}}{(k-1)!}\\[2ex]
			\vdots & \vdots & \ddots & \vdots\\
			0 & 0 & 0 & \dots & \dfrac{t^2}{2!}\\[2ex]
			0 & 0 & 0 & \dots & \dfrac{t}{1!}\\[2ex]
			0 & 0 & 0 & \dots & 1
		\end{pmatrix}
		\end{gathered}
	\end{equation*}
	Таким образом,
	\begin{equation*}
		e^{At} = e^{Bt}\cdot e^{\alpha t}\cdot E = e^{\alpha t}\cdot e^{Bt}.
	\end{equation*}
\end{Proof}
\begin{theorem}
	Пусть $A$ -- это произваольная матрица, $J$ -- это ее жорданова нормальная форма, а $S$ -- матрица трансформирующая $A$ в $J$, то есть $J = S^{-1}AS$. Тогда
	\begin{equation*}
		e^{At} = Se^{Jt}S^{-1}.
	\end{equation*}
\end{theorem}

\noindent\textbf{Замечание.} Так как $e^{At}$ -- это фундаментальная матрица $Dx = Ax$, а $S$ -- это невырожденная матрица, то матрица $e^{At}\cdot S = S e^{Jt}S^{-1} S = S e^{Jt}$ также является фундаментальной, если $J \in \mathbb R_{n,n}$.
...
\section{Уравнения, приводящиеся к уравнению с разделяющимися переменными. Однородное уравнение.}
...
Рассмотрим уравнение  $$y' = f\Big(\dfrac{a_1x + b_1y + c_1}{a_2 x + b_2y + c_2}\Big),\ c_1^2 + c_2^2 \ne 0.$$
\begin{enumerate}
