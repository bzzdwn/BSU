...
def estimate2(y, yl, x1, x2, poly1='beta',poly2='beta'):
    ...
    # Note: modified to allow for ylag=0
    if yl is not None:
        c = np.linalg.lstsq(np.concatenate([np.ones((len(xw1), 1)), xw1.reshape((len(xw1), 1)), xw2.reshape((len(xw2), 1)), yl], axis=1), y, rcond=None)[0]
        ...
        c = np.linalg.lstsq(np.concatenate([np.ones((len(xw1), 1)), xw1.reshape((len(xw1), 1)), xw2.reshape((len(xw2), 1))], axis=1), y, rcond=None)[0]
        #beta0,beta1,beta2,lambda
